...
<script>
  export default {
    ...
    computed:{
      ...
      circles(){
        ...
        for (let i = 0; i < this.empty(); i++){
            circles.push({
                cx: this.circleRadius + this.spacing() * this.filled + this.spacing() * i,
                cy: this.circleRadius,
                r: this.circleRadius - 1,
      ...
      lines(){
        ...
        for (let i = 0; i < linesEmpty; i++){
            lines.push({
                x1: this.circleRadius + linesFilled * this.spacing() + this.spacing() * i,
                y1: this.circleRadius,
                x2: this.circleRadius + linesFilled * this.spacing() + this.spacing() * (i + 1),
                y2: this.circleRadius,
    ...
    methods:{
        ...
        spacing(){
            return (this.width - this.circleRadius * 2) / (this.filled +  this.empty() - 1)
        }
    },
