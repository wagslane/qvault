...
export default async function secureRandomNumber(minimum, maximum) {
  ...
  if (minimum == null) {
    throw "You must specify a minimum value.";
  }

  if (maximum == null) {
    throw "You must specify a maximum value.";
  }

  if (minimum % 1 !== 0) {
    throw "The minimum value must be an integer.";
  }

  if (maximum % 1 !== 0) {
    throw "The maximum value must be an integer.";
  }

  if (!(maximum > minimum)) {
    throw "The maximum value must be higher than the minimum value.";
  }
  ...
  if (minimum < -9007199254740991 || minimum > 9007199254740991) {
    throw "The minimum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.";
  }

  if (maximum < -9007199254740991 || maximum > 9007199254740991) {
    throw "The maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.";
  }

  let range = maximum - minimum;

  if (range < -9007199254740991 || range > 9007199254740991) {
    throw "The range between the minimum and maximum value must be inbetween MIN_SAFE_INTEGER and MAX_SAFE_INTEGER.";
  }
