...
async function addLicenseToDmg(packager, dmgPath) {
  ...
  $"0000 0000 0000"
    ...
    const fileData = await (0, _fsExtraP().readFile)(item.file, "utf-8");
    const isRtf = item.file.endsWith(".rtf") || item.file.endsWith(".RTF");
    data += isRtf ? (0, _dmgUtil().serializeString)(Buffer.from(fileData).toString("hex")) : wrapInRtf((await (0, _fsExtraP().readFile)(item.file, "utf-8")));
    data += "\n};";
    rtfs.push(data);
  ...
  await (0, _fsExtraP().outputFile)(tempFile, data);
